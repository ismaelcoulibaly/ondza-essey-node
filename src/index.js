      ...
      schemas: {
        Reservation: {
          ...
          properties: {
            ...
            message: { type: 'string' },
            reservationType: { type: 'string', enum: ['CULINARY_MASTERCLASSES', 'EXCLUSIVE_DININGS', 'TAILORED_CHEF_SERVICES'] },
            phone: { type: 'string' },
            dateOfRequest: { type: 'string', format: 'date-time' },
            dateOfEvent: { type: 'string', format: 'date-time' },
            numberOfGuests: { type: 'integer', format: 'int32' },
          },
        },
        Subscriber: {
          type: 'object',
...
app.get('/admin', (req, res) => {
  Reservation.find().sort({ dateOfCreation: -1 })
    .then(reservations => res.render('admin', { reservations }))
